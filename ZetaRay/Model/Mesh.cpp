...
void PrimitiveMesh::ComputeGrid(Util::Vector<Vertex, App::ThreadAllocator>& vertices, Vector<INDEX_TYPE, App::ThreadAllocator>& indices,
...
void PrimitiveMesh::ComputeSphere(Vector<Vertex, App::ThreadAllocator>& vertices, Vector<INDEX_TYPE, App::ThreadAllocator>& indices,
	float diameter, size_t tessellation) noexcept
{
	...
	for (size_t i = 0; i < verticalSegments; i++)
	{
		for (size_t j = 0; j <= horizontalSegments; j++)
		{
			...
			indices.push_back((INDEX_TYPE)(i * stride + j));
			indices.push_back((INDEX_TYPE)(i * stride + nextJ));
			indices.push_back((INDEX_TYPE)(nextI * stride + j));

			indices.push_back((INDEX_TYPE)(i * stride + nextJ));
			indices.push_back((INDEX_TYPE)(nextI * stride + nextJ));
			indices.push_back((INDEX_TYPE)(nextI * stride + j));
		}
	}
...
void PrimitiveMesh::ComputeCylinder(Vector<Vertex, App::ThreadAllocator>& vertices, Vector<INDEX_TYPE, App::ThreadAllocator>& indices,
...
void PrimitiveMesh::ComputeCone(Vector<Vertex, App::ThreadAllocator>& vertices, Vector<INDEX_TYPE, App::ThreadAllocator>& indices,
	float diameter, float height, size_t tessellation) noexcept
{
	...
	for (size_t i = 0; i <= tessellation; i++)
	{
		...
		indices.push_back((INDEX_TYPE)(i * 2));
		indices.push_back((INDEX_TYPE)((i * 2 + 3) % (stride * 2)));
		indices.push_back((INDEX_TYPE)((i * 2 + 1) % (stride * 2)));
	}
	...
	for (size_t i = 0; i < tessellation - 2; i++)
	{
		...
		indices.push_back((INDEX_TYPE)vbase);
		indices.push_back((INDEX_TYPE)(vbase + i1));
		indices.push_back((INDEX_TYPE)(vbase + i2));
	}
...
void PrimitiveMesh::ComputeTorus(Vector<Vertex, App::ThreadAllocator>& vertices, Vector<INDEX_TYPE, App::ThreadAllocator>& indices,
...
namespace
{
	...
	void CreatePatchIndices(Vector<INDEX_TYPE, App::ThreadAllocator>& indices, size_t vbase, size_t tessellation, bool isMirrored)
	{
		size_t stride = tessellation + 1;
		...
		for (size_t i = 0; i < tessellation; i++)
		{
			for (size_t j = 0; j < tessellation; j++)
			{
				...
				{
					for (int i = 0; i < 6; i++)
						indices.push_back((INDEX_TYPE)(vbase + indexVals[i]));
				}
				else
				{
					for (int i = 5; i >= 0; i--)
	...
	void TessellatePatch(Vector<Vertex, App::ThreadAllocator>& vertices, Vector<INDEX_TYPE, App::ThreadAllocator>& indices,
...
void PrimitiveMesh::ComputeTeapot(Vector<Vertex, App::ThreadAllocator>& vertices, Vector<INDEX_TYPE, App::ThreadAllocator>& indices,
