...
void StaticBLAS::Rebuild(ComputeCmdList& cmdList) noexcept
{
	...
	Assert(currInstance == scene.m_numStaticInstances, "Invalid number of instances.");
...
void TLAS::RebuildTLAS(ComputeCmdList& cmdList) noexcept
{
	...
	for (int treeLevelIdx = 1; treeLevelIdx < scene.m_sceneGraph.size(); treeLevelIdx++)
	{
		...
		for (int i = 0; i < rtFlagVec.size(); i++)
		{
			...
			if (flags.MeshMode != RT_MESH_MODE::STATIC)
			{
				instance.InstanceID = numStaticInstances + std::max(currInstance - 1, 0);	// -1 is for not counting static blas instance
				...
				instance.AccelerationStructure = m_dynamicBLASes[std::max(currInstance - 1, 0)].m_blasBuffer.GetGpuVA();

				auto& M = currTreeLevel.m_toWorlds[i];

				for (int i = 0; i < 4; i++)
				{
					instance.Transform[0][i] = M.m[i].x;
					instance.Transform[1][i] = M.m[i].y;
					instance.Transform[2][i] = M.m[i].z;
				}
