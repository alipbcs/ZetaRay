...
namespace ZetaRay
{
	...
	namespace RenderPass
	{
		struct GBufferPass final
		{
			...
			void SetInstances(Vector<InstanceData>&& instances) noexcept;
		...
		private:
			...
			SmallVector<DrawCallArgs> m_perDrawCallArgs;

			// constant-buffer containing all the Per-draw data
			UploadHeapBuffer m_perDrawCB;
			...
			inline static const char* COMPILED_VS[] = { "GBufferPass_vs.cso" };
			inline static const char* COMPILED_PS[] = { "GBufferPass_ps.cso" };
		};
	}
}