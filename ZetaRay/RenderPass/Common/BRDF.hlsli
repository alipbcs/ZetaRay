...
SurfaceInteraction ComputePartialSurfaceInteraction(float3 sn, float roughness, float metalness,
	float3 wo, float3 baseColor)
{
	SurfaceInteraction surfaceInteraction;
	
	surfaceInteraction.wo = wo;
	surfaceInteraction.shadingNormal = sn;
	//surfaceInteraction.geometricNormal = gn;
	surfaceInteraction.ndotwo = clamp(dot(sn, wo), 0.00001f, 1.0f);
	surfaceInteraction.alpha = roughness * roughness;
	surfaceInteraction.alphaSq = max(0.00001f, surfaceInteraction.alpha * surfaceInteraction.alpha);
	surfaceInteraction.diffuseReflectance = baseColor * (1.0f - metalness);
	surfaceInteraction.F0 = lerp(float3(0.04f, 0.04f, 0.04f), baseColor, metalness);

	return surfaceInteraction;
}

// Completes computation of SurfaceInteractionData after wi has become known
void CompleteSurfaceInteraction(in float3 wi, inout SurfaceInteraction surfaceInteraction)
{
	surfaceInteraction.wh = normalize(wi + surfaceInteraction.wo);
	surfaceInteraction.ndotwi = clamp(dot(surfaceInteraction.shadingNormal, wi), 0.00001f, 1.0f);
	surfaceInteraction.whdotwo = saturate(dot(surfaceInteraction.wh, surfaceInteraction.wo)); // == hdotwi
	surfaceInteraction.ndotwh = saturate(dot(surfaceInteraction.shadingNormal, surfaceInteraction.wh));
	...
	surfaceInteraction.F = FresnelSchlick(surfaceInteraction.F0, surfaceInteraction.whdotwo);
}
...
float3 SampleSpecularBRDFGGXSmith(SurfaceInteraction surfaceInteraction, float2 u)
{
	...
	float3 woLocal = RotateVector(surfaceInteraction.wo, q);
	
	float3 whLocal = SampleGGXVNDF(woLocal,
		surfaceInteraction.alpha,
		surfaceInteraction.alpha,
		u);

	// transform wh from local space to world space
	float4 qReverse = q * float4(-1.0f, -1.0f, -1.0f, 1.0f);
	float3 whWorld = RotateVector(whLocal, qReverse);
