...
groupshared float g_horizontallyFiltered[NumToLoadPerRowOrColumn][8];
...
void HorizontalPass(uint3 Gid, uint Gidx)
{
	...
	RWTexture2D<half> g_lumVar = ResourceDescriptorHeap[g_local.SpatialLumVarDescHeapIdx];
	...
	for (int step = 0; step < 4; step++)
	{
		...
		if (Gidx4x16.x < 16 && IsInRange(pixelAddr, screenDim))
			val = g_lumVar[pixelAddr];
		...
		// row
		float filtered = Gidx4x16.x < GroupDim.x ? val * Kernel[0] : 0.0f;
		...
		for (int i = 0; i < Radius; i++)
		{
			...
			const float neighborVal = WaveReadLaneAt(val, neighborAddr);          
...
void VerticalPass(uint3 DTid, uint3 GTid)
{
	...
	RWTexture2D<half> g_variance = ResourceDescriptorHeap[g_local.SpatialLumVarFilteredDescHeapIdx];
	g_variance[DTid.xy] = half(filtered);
}
...
void foo(uint3 DTid)
{
	const int2 screenDim = float2(g_frame.RenderWidth, g_frame.RenderHeight);
	...
	for (int i = -1; i <= 1; i++)
	{
		for (int j = -1; j <= 1; j++)
		{
			int2 addr = DTid.xy + int2(i, j);
			
			if (IsInBounds(addr, screenDim))
			{
				float val = g_lumVar[addr];
				float k = Kernel[i + 1] * Kernel[j + 1];
				filtered += val * k;
			}
		}
	}
	
	RWTexture2D<half> g_out = ResourceDescriptorHeap[g_local.SpatialLumVarFilteredDescHeapIdx];
	g_out[DTid.xy] = half(filtered);
...
void main(uint3 DTid : SV_DispatchThreadID, uint3 Gid : SV_GroupID, uint Gidx : SV_GroupIndex, uint3 GTid : SV_GroupThreadID)
{
	...
	const half isSurfaceMarker = g_baseColor[DTid.xy].w;
