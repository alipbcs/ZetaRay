	...
	: m_rootSig(NUM_CBV, NUM_SRV, NUM_UAV, NUM_GLOBS, NUM_CONSTS)
{
	...
	m_rootSig.InitAsConstants(0,						// root idx
		(int)std::max(sizeof(cbTemporalPass) / sizeof(DWORD), sizeof(cbSpatialPass) / sizeof(DWORD)),			// num-DWORDs
		1,												// register
		0);												// register-space
...
void ReSTIR_GI::Render(CommandList& cmdList) noexcept
{
	...
	// Temporal resampling/Validation
	{
		...
		m_cbTemporal.DispatchDimX = dispatchDimX;
		m_cbTemporal.DispatchDimY = dispatchDimY;
	...
	if (m_doSpatialResampling)
	{
		...
		m_cbSpatial.DispatchDimX = dispatchDimX;
		m_cbSpatial.DispatchDimY = dispatchDimY;
