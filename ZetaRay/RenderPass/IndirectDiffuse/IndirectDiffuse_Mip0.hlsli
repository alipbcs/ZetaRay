...
//	if (shouldThisLaneTrace)
	...
	if (shouldThisLaneTrace)
	{
		RWTexture2D<uint4> outLiRayT = ResourceDescriptorHeap[g_local.OutputDescHeapIdx];
		
#if defined(PROJECT_TO_SH_BASIS)
		float3 wi = g_sortedDir[Gidx];
		const half3 valYCoCg = RGBToYCoCg((half3) val.rgb);
		
		half4 lumaSH;
		lumaSH.x = (half) SHBasis00 * valYCoCg.x;
		lumaSH.y = (half) SHBasis1_1(wi) * valYCoCg.x;
		lumaSH.z = (half) SHBasis10(wi) * valYCoCg.x;
		lumaSH.w = (half) SHBasis11(wi) * valYCoCg.x;
		
		uint4 ret;
		ret.x = (asuint16(lumaSH.y) << 16) | asuint16(lumaSH.x);
		ret.y = (asuint16(lumaSH.w) << 16) | asuint16(lumaSH.z);
		ret.z = (asuint16(valYCoCg.z) << 16) | asuint16(valYCoCg.y);
		ret.w = asuint(LuminanceFromLinearRGB(val.rgb));
		outLiRayT[swizzledDTid].xyz = ret;
#else
		uint2 ret;
