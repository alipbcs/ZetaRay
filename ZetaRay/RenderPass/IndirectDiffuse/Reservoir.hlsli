...
struct Reservoir
{
	...
	static Reservoir Init(float3 samplePos, float w_sum, half3 Li, half M, half2 sampleNormal)
	{
		Reservoir res;
...
float JacobianDeterminant(float3 x1_q, float3 x2_q, float3 wi, float3 secondToFirst_r, Reservoir neighborReservoir)
{
...
float3 PackNormalLiRayT(half2 normal, float3 Li, half rayT)
{
	uint3 r;
	half3 Lih = half3(Li);
...
Reservoir ReadInputReservoir(uint2 DTid, uint inputAIdx, uint inputBIdx, uint inputCIdx)
{
	...
	Reservoir r = Reservoir::Init(resA.xyz, resA.w, resB.xyz, resB.w, resC);

	return r;
}

Reservoir ReadInputReservoir(SamplerState s, float2 uv, uint inputAIdx, uint inputBIdx, uint inputCIdx)
{
	Texture2D<float4> g_inReservoir_A = ResourceDescriptorHeap[inputAIdx];
	Texture2D<half4> g_inReservoir_B = ResourceDescriptorHeap[inputBIdx];
	Texture2D<half2> g_inReservoir_C = ResourceDescriptorHeap[inputCIdx];
...
Reservoir PartialReadInputReservoir(uint2 DTid, uint inputAIdx, uint inputBIdx)
{
	...
	Reservoir r = Reservoir::Init(resA.xyz, resA.w, resB.xyz, resB.w, 0.0.xx);

	return r;
}

Reservoir PartialReadInputReservoir(SamplerState s, float2 uv, uint inputAIdx, uint inputBIdx)
{
	Texture2D<float4> g_inReservoir_A = ResourceDescriptorHeap[inputAIdx];
	Texture2D<half4> g_inReservoir_B = ResourceDescriptorHeap[inputBIdx];
...
void WriteOutputReservoir(uint2 DTid, Reservoir r, uint outputAIdx, uint outputBIdx, uint outputCIdx)
{
