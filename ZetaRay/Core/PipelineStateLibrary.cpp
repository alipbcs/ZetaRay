...
void PipelineStateLibrary::Init(const char* name) noexcept
{
	...
	if (m_foundOnDisk)
	{
		...
		{
			if (hr == E_INVALIDARG)
				LOG("PSO cache %s is corrupted.\n", m_psoLibPath1.Get());

			if (hr == D3D12_ERROR_DRIVER_VERSION_MISMATCH)
				LOG("PSO cache %s has driver mismatch.\n", m_psoLibPath1.Get());

			if (hr == D3D12_ERROR_ADAPTER_NOT_FOUND)
				LOG("PSO cache %s was created using a different hardware than the one being used right now.\n", m_psoLibPath1.Get());
	...
	const char* pathToCompiledPS) noexcept
{
	...
	if (!pso)
	{
		...
		SmallVector<uint8_t, App::PoolAllocator> vsBytecode;
		SmallVector<uint8_t, App::PoolAllocator> psBytecode;
	...
	const char* pathToCompiledCS) noexcept
{
	...
	if (!pso)
	{
		...
		SmallVector<uint8_t, App::PoolAllocator> bytecode;
