	...
	float diameter, size_t tessellation)
{
	...
	for (size_t i = 0; i <= verticalSegments; i++)
	{
		...
		for (size_t j = 0; j <= horizontalSegments; j++)
		{
			...
			vertices.emplace_back(normal * radius, textureCoordinate, snorm3(normal), snorm3(tangent));
		}
	}
	...
	float bottomRadius, float topRadius, float height, uint32_t sliceCount, uint32_t stackCount)
{
	...
	for (uint32_t i = 0; i < ringCount; ++i)
	{
		...
		for (uint32_t j = 0; j <= sliceCount; ++j)
		{
			...
			vertices.emplace_back(pos, uv, snorm3(normal), snorm3(tangent));
		}
	}
	...
	for (uint32_t i = 0; i <= sliceCount; ++i)
	{
		...
		vertices.emplace_back(float3(x, y, z), float2(u, v), snorm3(0.0f, 1.0f, 0.0f), snorm3(1.0f, 0.0f, 0.0f));
	}

	// Cap center vertex.
	vertices.emplace_back(float3(0.0f, y, 0.0f), float2(0.5f, 0.5f), snorm3(0.0f, 1.0f, 0.0f), snorm3(1.0f, 0.0f, 0.0f));
	...
	float diameter, float height, size_t tessellation)
{
	...
	for (size_t i = 0; i <= tessellation; i++)
	{
		...
		vertices.emplace_back(topOffset, float2(0.0f, 0.0f), snorm3(normal), snorm3(0.0f, 0.0f, 0.0f));
		vertices.emplace_back(pt, float2(u, 1.0f), snorm3(normal), snorm3(0.0f, 0.0f, 0.0f));
	...
	for (size_t i = 0; i < tessellation; i++)
	{
		...
		vertices.emplace_back(position, textureCoordinate, snorm3(normal), snorm3(0.0f, 0.0f, 0.0f));
	}
	...
	for (size_t i = 0; i <= tessellation; i++)
	{
		...
		for (size_t j = 0; j <= tessellation; j++)
		{
			...
			vertices.emplace_back(float3(position.x, position.y, position.z), textureCoordinate, 
				snorm3(normal.x, normal.y, normal.z), snorm3(0.0f, 0.0f, 0.0f));
