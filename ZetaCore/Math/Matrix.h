...
namespace ZetaRay::Math
{
	...
	struct float4x3
	{
		...
		float4x3(const float3& row0, const float3& row1, const float3& row2, const float3& row3) noexcept
		{
			m[0] = row0;
			m[1] = row1;
			m[2] = row2;
		...
		explicit float4x3(const float4x4a& M) noexcept;
	...
	struct float3x4
	{
		...
		float3x4(const float4& row0, const float4& row1, const float4& row2) noexcept
		{
			m[0] = row0;
			m[1] = row1;
			m[2] = row2;
		...
		explicit float3x4(const float4x3& M) noexcept
		{
		...
		explicit float3x4(const float4x4a& M) noexcept;
	...
	struct alignas(16) float4x4a
	{
		...
		float4x4a(const float4& row0, const float4& row1, const float4& row2, const float4& row3) noexcept
		{
			m[0] = row0;
			m[1] = row1;
			m[2] = row2;
		...
		explicit float4x4a(const float* arr) noexcept
		{
		...
		explicit float4x4a(const double* arr) noexcept
		{
		...
		explicit float4x4a(const float4x3& M) noexcept
		{
	...
	struct float3x3
	{
		...
		explicit float3x3(const float4x4a& M) noexcept
		{
		...
		explicit float3x3(const float4x3& M) noexcept
		{
			m[0] = float3(M.m[0].x, M.m[0].y, M.m[0].z);
			m[1] = float3(M.m[1].x, M.m[1].y, M.m[1].z);
			m[2] = float3(M.m[2].x, M.m[2].y, M.m[2].z);
	...
	struct v_float4x4
	{
		v_float4x4() noexcept = default;

		v_float4x4(__m128 row0, __m128 row1, __m128 row2, __m128 row3) noexcept
	...
	inline float4x3::float4x3(const float4x4a& M) noexcept
	{
		m[0] = float3(M.m[0].x, M.m[0].y, M.m[0].z);
		m[1] = float3(M.m[1].x, M.m[1].y, M.m[1].z);
		m[2] = float3(M.m[2].x, M.m[2].y, M.m[2].z);
	...
	inline float3x4::float3x4(const float4x4a& M) noexcept
	{
		m[0] = float4(M.m[0].x, M.m[1].x, M.m[2].x, M.m[3].x);
		m[1] = float4(M.m[0].y, M.m[1].y, M.m[2].y, M.m[3].y);
		m[2] = float4(M.m[0].z, M.m[1].z, M.m[2].z, M.m[3].z);
